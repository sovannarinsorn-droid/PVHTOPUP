...
STATIC_DIR = BASE_DIR  # index_v6.html + admin_v4.html live alongside this file
...
def fazercards_telegram_stars_buy(telegram_username, quantity, idempotency_key):
    """Auto-deliver Telegram Stars directly to an account: POST /telegram/stars/buy.
    Unlike gift cards, this is a genuine direct-to-account top-up (like FF/ML
    diamonds) — no redemption code, no manual step. quantity must be 50-10000
    per FazerCards' documented range."""
    if not FAZERCARDS_API_KEY:
        raise FazerCardsError("FAZERCARDS_API_KEY is not set")
    res = requests.post(
        f"{FAZERCARDS_BASE_URL}/telegram/stars/buy",
        json={"telegram_username": telegram_username, "quantity": quantity},
        headers=_fazercards_headers(idempotency_key),
        timeout=20,
    )
    try:
        ...
        raise FazerCardsError(f"Bad response from FazerCards ({res.status_code})")
    if not data.get("ok"):
        raise FazerCardsError(data.get("error") or f"Telegram Stars order failed ({res.status_code})")
    return data


def fazercards_telegram_premium_buy(telegram_username, months, idempotency_key):
    """Auto-deliver Telegram Premium directly to an account: POST /telegram/premium/buy.
    months must be 3, 6, or 12 per FazerCards' documented plans."""
    if not FAZERCARDS_API_KEY:
        raise FazerCardsError("FAZERCARDS_API_KEY is not set")
    res = requests.post(
        f"{FAZERCARDS_BASE_URL}/telegram/premium/buy",
        json={"telegram_username": telegram_username, "months": months},
        headers=_fazercards_headers(idempotency_key),
        timeout=20,
    )
    try:
        ...
        raise FazerCardsError(f"Bad response from FazerCards ({res.status_code})")
    if not data.get("ok"):
        raise FazerCardsError(data.get("error") or f"Telegram Premium order failed ({res.status_code})")
    return data
...
def check_payment():
    ...
    def _mutate(d):
        ...
        fc_offer_id = (product or {}).get("provider_package")  # FazerCards offer_id / card_id / qty-months lives here
        fulfillment_type = (game or {}).get("fulfillment_type") or "topup"
        is_giftcard = fulfillment_type == "giftcard"
        is_telegram = fulfillment_type in ("telegram_stars", "telegram_premium")
        ...
        # Telegram Stars/Premium need no category_id at all — just the API key and
        # the quantity/months stored in provider_package. Everything else (topup,
        # giftcard) still needs a FazerCards category to know which catalog to hit.
        can_attempt = FAZERCARDS_API_KEY and fc_offer_id and (is_telegram or fc_category)
        ...
        if can_attempt:
            try:
                ...
                if is_telegram:
                    if fulfillment_type == "telegram_stars":
                        fc_res = fazercards_telegram_stars_buy(o["user_id"], int(fc_offer_id), idempotency_key=o["trx_id"])
                    else:
                        fc_res = fazercards_telegram_premium_buy(o["user_id"], int(fc_offer_id), idempotency_key=o["trx_id"])
                elif is_giftcard:
                ...
                delivery_status = "failed"
                delivery_error = f"Invalid quantity/months in provider_package: {e}"
