...
def fazercards_validate_id(fields_category_id, validate_category_id, user_id, zone_id=None):
    """Auto CHECK ID: POST /topups/validate-id -> {ok, valid, player_name, region?}

    IMPORTANT (confirmed live against the FazerCards API): /topups/validate-id
    uses its OWN category_id namespace, which is NOT the same as the
    category_id used for /topups/offers and /topups/order. For Free Fire,
    offers/order use "free_fire_my_sg" while validate-id uses "free_fire".
    Using one category_id for both was causing wrong packages/prices on the
    storefront and "account not found" errors even for valid Player IDs.
    That's why this takes two separate category ids: `fields_category_id`
    (used only to look up which fields — player_id, zone_id — this game
    needs, via /topups/offers, which is reliable) and `validate_category_id`
    (the actual category_id sent to /topups/validate-id)."""
    if not FAZERCARDS_API_KEY:
        raise FazerCardsError("FAZERCARDS_API_KEY is not set")
    fields = fazercards_build_fields(fields_category_id, user_id, zone_id)
    res = requests.post(
        f"{FAZERCARDS_BASE_URL}/topups/validate-id",
        json={"category_id": validate_category_id, "fields": fields},
        headers=_fazercards_headers(),
        timeout=15,
    )
    try:
...
@app.route("/admin/")
def serve_admin():
    return send_from_directory(STATIC_DIR, "admin_v3.html")
...
# Known FazerCards category_id mismatches between /topups/offers (used for the
# storefront package list + auto top-up) and /topups/validate-id (used for
# auto check-ID). Confirmed live 2026-07-08. Lets these games' check-ID work
# immediately even before an admin sets fazercards_validate_category_id by hand
# in the admin panel — new games can be added here as they're confirmed.
_FAZERCARDS_VALIDATE_CATEGORY_FALLBACK = {
    "free_fire_my_sg": "free_fire",
    "mobile_legends_global": "mobile_legends",
    "pubg_mobile_auto": "pubg_mobile",
}


def _resolve_validate_category(fc_category, fc_validate_category):
    if fc_validate_category:
        return fc_validate_category
    return _FAZERCARDS_VALIDATE_CATEGORY_FALLBACK.get(fc_category, fc_category)
...
def check_user():
    ...
    fc_category = (game or {}).get("fazercards_category_id")
    # FazerCards' validate-id endpoint uses its own category_id namespace,
    # different from the one used for offers/order (e.g. "free_fire" vs
    # "free_fire_my_sg" for Free Fire) — see fazercards_validate_id() above.
    # Falls back to fc_category if this game hasn't been given its own
    # validate-id category yet (some games may happen to share one namespace).
    fc_validate_category = _resolve_validate_category(
        fc_category, (game or {}).get("fazercards_validate_category_id")
    )
    ...
    try:
        result = fazercards_validate_id(fc_category, fc_validate_category, user_id, zone_id)
    except FazerCardsError as e:
...
def admin_games():
    # fazercards_category_id: the FazerCards topup category for OFFERS/ORDER
    # (enables auto top-up). Get it from GET /topups/offers?category_id=... —
    # for Free Fire this is "free_fire_my_sg", confirmed live 2026-07-08.
    #
    # fazercards_validate_category_id: the category for the /topups/validate-id
    # CHECK-ID call. FazerCards uses a SEPARATE category_id namespace here —
    # for Free Fire this is "free_fire" (get it from GET /topups/validate-id).
    # If left blank, falls back to fazercards_category_id, which only happens
    # to work for games where both namespaces coincide.
    return _admin_crud(
        "games",
        ["name", "code", "image_url", "fazercards_category_id", "fazercards_validate_category_id"],
